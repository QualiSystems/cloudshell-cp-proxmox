...
class AbstractProxmoxDeployFlow(AbstractDeployFlow):
    ...
    def _get_connectors(self, vm_name):
        """Get connectors."""
        result = []
        with suppress(Exception):
            sandbox = self._cs_api.GetReservationDetails(
                self._reservation_info.reservation_id
            ).ReservationDescription
            result = [x for x in sandbox.Connectors if x.Target.lower() ==
                      vm_name.lower()
                    or x.Source.lower() == vm_name.lower()]
        return result
    ...
    def _create_vm(
        ...
        return CloneVMCommand(
            ...
            copy_src_uuid=deploy_app.copy_src_uuid,
        ).execute()
    ...
    def _deploy(self, request_actions: DeployVMRequestActions) -> DeployAppResult:
        ...
        name = deploy_app.app_name.replace(" ", "-")
        connectors = self._get_connectors(name)
        if deploy_app.autogenerated_name:
            instance_name = self.generate_name(name)
        else:
            instance_name = name
        ...
        with self._rollback_manager:
            ...
            vm = InstanceConfig.from_proxmox_instance(
                self.proxmox_api,
                deployed_vm_id
            )
            if vm.interfaces:
                interface = vm.interfaces[0]
                if len(connectors) > len(vm.interfaces):
                    for number, connector in enumerate(connectors):
                        self.proxmox_api.attach_interface(
                            network_bridge=interface.get(
                                "bridge",
                                self._resource_config.default_bridge
                            ),
                            instance_id=deployed_vm_id,
                            vlan_tag=interface.get("tag"),
                            interface_type=interface.get(
                                "type",
                                "virtio"
                            ),
                            vnic_id=number
